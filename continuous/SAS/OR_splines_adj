...
** DATE:    April 2025
** ----------------------------------------------------------------
** PURPOSE: Odds Ratio - Logistic Regression Model
...
*** GRPDIFF = Specify (1) for odds of group 1 vs group 2 (single line);
...
*** REFLINEODDS = Requests a line of reference for the odds ratio drawn at 1;
...
*** MACRO starts here;
*** #############################################################;
%macro LOGISTIC_ODDS_RCS_ADJ(indat=, trim=, censvar=, predvar=, refval=, adjcat=, adjcov=, knotmethod=, ptknots=, grpdiff=, grpref=, bygrp=, bygrpnm1=, bygrpnm2=, color1=, color2=, 
                                xlabel=, xlabelsize=, xvals=, ylabel=, ylabelsize=, yvals=, y2label=, y2labelsize=, y2vals=, title=, knotlines=, refline=, reflineodds=, hist=, nbins=, grid=, linear=);
...
%end;
	model &censvar. = splxvar &adjcov. / link=logit;
	estimate splxvar[-1,&refval.][1,&k.] / exp cl;
	ods output Estimates=orD;
run;
ods exclude close;
/*proc print data=orD(obs=10); run;*/

data orAD; 
	set orD;
atval = &k.;
run;
/*proc print data=orAD; run;*/

proc append base=orZD 
	data=orAD force;
run;

/*proc print data=orZD;*/
/*run;*/
...
%if &knotmethod. = 2 %then %do;
	effect splxvar = spline(&predvar. / naturalcubic knotmethod=RANGEFRACTIONS(&ptknots.));
%end;
	model &censvar. = splxvar|&bygrp. &adjcov. / link=logit;
	oddsratio &bygrp. / at(&predvar.=&k.) diff=ref;
	ods output OddsRatiosWald=orD;
run;
ods exclude close;
/*proc print data=orD(obs=10); run;*/
...
%end;
%if &grpdiff.=1 %then %do;
      estimA1 = OddsRatioEst;
      lowerestimA1 = LowerCL;
   	  upperestimA1 = UpperCL;
%end;
...
%if &reflineodds. = 1 %then %do;
referenceline y=1 / lineattrs=(color=black pattern=Dash);
%end;
